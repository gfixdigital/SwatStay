...
| 18 Sep 2026 | Project lead | Connected the protected Media Library to package and destination editors with reusable cover/gallery selection. Public package and destination responses now convert private media paths into temporary signed URLs. | Run authenticated admin media-picker QA with a real catalog edit, then connect provider-specific media records. |
| 18 Sep 2026 | Project lead | Added additive provider media storage for cover image, gallery images, and documents. Added protected admin/operations GET/PATCH provider media endpoints, private-path validation, signed previews, audit logging, and migration. Provider dashboard files were not changed. | Add provider admin media controls after the provider record list is connected to live API data. |
| 18 Sep 2026 | Project lead | Connected the admin provider directory to live provider records, status updates, provider detail loading, and reusable private media controls for cover images, galleries, and documents. Provider dashboard files and teammate-owned workflows were not changed. | Test authenticated provider status and media updates with a real admin account, then continue with provider assignment workflow QA. |
| 18 Sep 2026 | Project lead | Connected admin provider assignment to live booking records. Added approved-provider suggestions by service type, provider assignment/reassignment, required-service validation, payment verification gating, provider-pending transition, booking events, and audit records. Provider dashboard files were not changed. | Test the complete authenticated flow with a real verified-payment booking and approved providers. Provider notification and provider-side acceptance remain later work. |
| 18 Sep 2026 | Project lead | Added provider handoff API foundation without changing the provider dashboard UI. Providers can securely list their assigned booking items and accept or reject each assignment. Decisions create booking/audit events, move fully accepted trips to confirmed, and route rejected assignments to admin action. | Connect the existing provider dashboard UI to these endpoints and add provider notification delivery later. |
| 18 Sep 2026 | Project lead | Connected the provider dashboard assignment queue to the provider handoff API when a provider access token exists. Live assignments replace the demo queue, accept/reject actions are persisted, and the dashboard keeps a safe demo fallback for preview accounts. Unrelated provider screens were preserved. | Add provider authentication/session UI and real notification delivery before production use. |
| 18 Sep 2026 | Project lead | Added provider login/session UI with provider-role validation, access/refresh token persistence, sign-out, live-session loading, and a clearly labeled preview workspace fallback. | Test login with an approved provider account, then add refresh-token renewal and production notification delivery. |
| 18 Sep 2026 | Project lead | Removed demo-trip behavior for new customer accounts, connected the public provider registration form to the API, removed sample finance/provider/support counts from the admin overview, and added a controlled `admin:create` setup command. Existing database records are preserved. | Run the controlled admin setup command with real credentials, then connect remaining admin sub-pages to live endpoints before production review. |
