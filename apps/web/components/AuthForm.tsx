...
export function AuthForm({ mode }: { mode: "login" | "signup" }) {
  ...
  function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setError(""); const data = new FormData(event.currentTarget); if (signup && data.get("password") !== data.get("confirmPassword")) { setError("Passwords do not match."); return; } const email = String(data.get("email") ?? "traveler@example.com"); const name = signup ? `${String(data.get("firstName") ?? "")} ${String(data.get("lastName") ?? "")}`.trim() : "Ayesha Khan"; setTraveler({ name: name || "Traveler", email, hasDemoTrip: !signup }); setSubmitted(true); window.setTimeout(() => router.push("/dashboard"), 350); }
