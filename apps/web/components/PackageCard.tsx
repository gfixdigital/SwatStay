...
export function PackageCard({ item, arc = false, position = "center" }: { item: TourPackage; arc?: boolean; position?: "left" | "center" | "right" }) {
  ...
  >
    <div className="overflow-hidden rounded-t-brand"><div className="relative h-52 bg-cover bg-center transition-transform duration-300 group-hover:scale-[1.01]" style={{ backgroundImage: `url(${item.image})` }}/></div>
    <div className="p-[18px]">
      ...
      <div className="mt-4 flex items-center justify-between border-t border-border pt-4"><div><small className="block text-[11px] text-stone">Starting from</small><strong className="font-display text-xl text-amber">PKR {item.price.toLocaleString()}</strong></div><Link href={`/packages/${item.slug}`} className="button min-h-[38px] border-border bg-white px-3 text-xs text-pine hover:bg-mist">View details</Link></div>
    </div>
  </motion.article>;
}
