...
import { ArrowRight, CheckCircle2, Loader2, MessageCircle, XCircle } from "lucide-react";
...
export function BookingForm({ packageTitle = "A SwatStay package", packageImage, packageSlug, destination }: { packageTitle?: string; packageImage?: string; packageSlug?: string; destination?: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!legalAccepted) { setLegalError("Please agree to the Terms and Conditions and Privacy Policy before submitting."); return; }
    setLegalError(""); setError(""); setSubmitting(true);
    ...
    const result = await createBooking({
      ...
      travelStart: String(data.get("travelStartDate") ?? ""),
      travelEnd: String(data.get("travelEndDate") ?? ""),
      travelersCount: Number(String(data.get("travelers") ?? "2").replace("+", "")),
      pickupCity: String(data.get("pickupCity") ?? ""),
      specialRequests: String(data.get("specialRequests") ?? "") || undefined,
      consent: true,
    }).catch((reason: unknown) => { setError(reason instanceof Error ? reason.message : "We could not submit your booking request."); return null; });
    if (!result) { setSubmitting(false); return; }
    setSubmitted(true);
    router.push(`/booking/success?reference=${encodeURIComponent(String((result as { reference?: string }).reference ?? ""))}`);
  }
  ...
  return (
    <form onSubmit={submit} className="grid gap-5 rounded-brand border border-border bg-white p-5 shadow-editorial md:p-7">
      ...
      {error && <div className="flex items-start gap-2 rounded-brand border border-red-200 bg-red-50 p-3 text-xs text-red-700"><XCircle size={16} className="mt-0.5 shrink-0" /><span>{error}</span></div>}

      <div className="grid gap-4 sm:grid-cols-2">
        <label className="text-xs font-semibold text-stone">Full name<input required name="fullName" className="field mt-1 w-full" placeholder="Your name"/></label>
        <CountryPhoneField/>
        <label className="text-xs font-semibold text-stone">Email address<input required type="email" name="email" className="field mt-1 w-full" placeholder="you@example.com"/></label>
        <CustomSelect label="Preferred call language" name="preferredLanguage" value="English" options={[{ label: "English", value: "English" }, { label: "Urdu", value: "Urdu" }, { label: "Chinese", value: "Chinese" }]}/>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <CustomSelect label="Destination" name="destination" value={destination ?? "Kalam"} options={[{ label: "Kalam", value: "Kalam" }, { label: "Malam Jabba", value: "Malam Jabba" }, { label: "Bahrain", value: "Bahrain" }, { label: "Anywhere in Swat", value: "all" }]}/>
        <CustomSelect label="Stay level" name="tier" value="standard" options={[{ label: "Basic", value: "basic" }, { label: "Standard", value: "standard" }, { label: "Premium", value: "premium" }]}/>
      </div>
      ...
      <button disabled={submitted || submitting} className="button min-h-12 bg-pine text-white hover:bg-[#0e2c22] disabled:opacity-70">{submitted ? "Request submitted" : submitting ? <><Loader2 size={16} className="animate-spin"/> Submitting...</> : <>Submit booking request <ArrowRight size={16}/></>}</button>
    </form>
  );
}
