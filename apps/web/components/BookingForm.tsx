...
export function BookingForm({
  ...
  packageImage,
  destination: defaultDestination = "Kalam",
}: {
  packageSlug?: string;
  packageTitle?: string;
  packageImage?: string;
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!legalAccepted) {
      setLegalError(
        "Please agree to the Terms and Conditions and Privacy Policy before submitting.",
      );
      return;
    }
    setLegalError("");
    ...
    const payload = {
      ...
      phone: data.get("phone")
        ? `${data.get("countryCode")}${data.get("phone")}`
        : undefined,
      whatsapp: data.get("phone")
        ? `${data.get("countryCode")}${data.get("phone")}`
        : undefined,
      country: data.get("country") as string,
      destination: (data.get("destination") as string) || defaultDestination,
      travelStart: (data.get("travelStartDate") as string) || undefined,
      travelEnd: (data.get("travelEndDate") as string) || undefined,
      travelersCount: parseInt(data.get("travelers") as string, 10) || 2,
      tier: (data.get("tier") as string) || undefined,
      pickupCity: (data.get("pickupCity") as string) || undefined,
      specialRequests:
        (data.get("specialRequests") as string) || undefined,
      preferredPaymentMethod:
        (data.get("paymentMethod") as string) || "call",
      preferredLanguage:
        (data.get("preferredLanguage") as string) || "English",
      packageSlug,
      packageTitle,
  ...
  return (
    <form
      ref={formRef}
      onSubmit={submit}
      className="grid gap-5 rounded-brand border border-border bg-white p-5 shadow-editorial md:p-7"
    >
      {packageImage && (
        <div className="-mx-5 -mt-5 overflow-hidden rounded-t-brand border-b border-border md:-mx-7 md:-mt-7">
          ...
          <div className="bg-mist px-4 py-3">
            <span className="text-[11px] font-semibold uppercase tracking-[.5px] text-river">
              REQUESTING
            </span>
            <strong className="mt-1 block font-display text-lg text-pine">
              {packageTitle}
            </strong>
          </div>
        </div>
      ...
      {successRef && (
        <div className="rounded-brand border border-pine/30 bg-pine/5 p-4 text-center">
          <CheckCircle2 className="mx-auto text-pine" size={28} />
          <p className="mt-2 text-sm font-semibold text-pine">
            Booking request received
          </p>
          <p className="mt-1 text-xs text-stone">
            Reference: <strong>{successRef}</strong> — redirecting...
          </p>
        </div>
      )}

      {error && (
        <div className="flex items-start gap-2 rounded-brand border border-red-200 bg-red-50 p-3 text-xs text-red-700">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <CustomSelect
          ...
          value="call"
          options={[
            { label: "Discuss on confirmation call", value: "call" },
      ...
      <div className="rounded-brand border border-border bg-mist p-4">
        <p className="flex gap-2 text-xs leading-5 text-stone">
          <CheckCircle2
            size={16}
            className="mt-0.5 shrink-0 text-river"
          />
          A team member will call to confirm availability, details, and
          payment before anything is booked.
        </p>
        <p className="mt-3 border-t border-border pt-3 text-xs leading-5 text-stone">
          Cancellation depends on provider policy and booking date. The GFix
          team confirms refund rules during the call. Weather and road changes
          may require rescheduling. Read the{" "}
