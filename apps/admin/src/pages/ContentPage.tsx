...
type ContentEntry = { id: string; entryKey: string; payload: ContentSection; status: string; version: number; updatedAt: string };
...
export function ContentPage() {
  ...
  useEffect(() => {
    ...
    void adminRequest<ContentEntry[]>("/admin/content?kind=website").then((entries) => {
      if (entries.length) setSections(entries.map((entry) => ({ ...entry.payload, id: entry.payload.id || entry.entryKey, status: entry.status === "PUBLISHED" ? "Published" : "Draft", version: entry.version, updatedAt: new Date(entry.updatedAt).toLocaleDateString(), updatedBy: "Current admin" })));
    }).catch(() => setToast("Could not load saved content. Showing the local draft."));
  }, [setSections]);

  async function persistSection(section: ContentSection, status: "DRAFT" | "PUBLISHED") {
    if (!getAdminToken()) return;
    try {
      await adminRequest(`/admin/content/${encodeURIComponent(`website-${section.id}`)}`, { method: "PUT", body: JSON.stringify({ kind: "website", status, payload: section }) });
    } catch (reason: unknown) {
      setToast(reason instanceof Error ? reason.message : "Could not save content to the API.");
      throw reason;
    }
  }
  ...
  async function saveDraft(event: FormEvent<HTMLFormElement>) {
    ...
    setSelected(updated);
    await persistSection(updated, "DRAFT");
  ...
  async function publish() {
    ...
    setSections((current) => current.map((section) => section.id === selected.id ? updated : section));
    setSelected(updated);
    await persistSection(updated, "PUBLISHED");
    setPublishPending(false);
    setToast(`${selected.title} published successfully.`);
  }
  ...
  async function createSection(event: FormEvent<HTMLFormElement>) {
    ...
    await persistSection(section, "DRAFT");
  ...
  return <>
    <PageHeader eyebrow="PUBLIC WEBSITE CONTENT" title="Site builder" description="Manage public copy, calls to action, consent text, legal content, and page sections. Save drafts and publish approved changes through the content API." actions={<button type="button" className="button-primary" onClick={() => setCreateOpen(true)}><Plus size={15}/> Add content section</button>}/>
    ...
    <ConfirmDialog open={publishPending} title="Publish website content?" message={`Publish the current ${selected?.title ?? "content"} values through the content API?`} confirmLabel="Publish content" onConfirm={publish} onClose={() => setPublishPending(false)}/>
    <AdminModal open={createOpen} onClose={() => setCreateOpen(false)} title="Add content section" description="Create a draft section for a new public website element. It will be saved to the content API when you submit."><form onSubmit={createSection} className="grid gap-4"><label className="label">Section title<input required className="field mt-1.5" value={newTitle} onChange={(event) => setNewTitle(event.target.value)} placeholder="Example: Package listing empty state"/></label><label className="label">Website area<select className="field mt-1.5" value={newArea} onChange={(event) => setNewArea(event.target.value as ContentSection["area"])}><option>Homepage</option><option>Packages</option><option>Destinations</option><option>Forms</option><option>Global</option><option>Help</option><option>Legal</option></select></label><button className="button-primary">Create draft section</button></form></AdminModal>
